...
void encode_treeview_cursor_changed_cb(GtkTreeView* tree_view, gpointer user_data)
{
	...
	GtkTextBuffer* text_buffer			= GLOBAL_ENCODING_DIALOG.textbuffer;
	char* src_buffer					= GLOBAL_ENCODING_DIALOG.src_text;
	char dst_buffer[6*ENCODE_BUFF_SIZE];
	size_t dst_buff_size				= sizeof(dst_buffer);
	size_t buffer_data_size				= GLOBAL_ENCODING_DIALOG.src_text_size;
	...
	if(gtk_tree_selection_get_selected(tree_selection, NULL, &tree_iter) == TRUE)
	{
		...
		if(res == (size_t)-1)
			gtk_text_buffer_set_text(text_buffer, _C("WRONG ENCODING"), -1);
		else
		{
...
void txt_encode_dialog_show_cb(GtkWidget *widget, gpointer user_data)
{
	...
	char* src_text			= GLOBAL_ENCODING_DIALOG.src_text;
	size_t* src_text_size	= &(GLOBAL_ENCODING_DIALOG.src_text_size);

	FILE* f = fopen(file_path, "rb");
	*src_text_size = fread(src_text, 1, sizeof(GLOBAL_ENCODING_DIALOG.src_text), f);
	fclose(f);
}
