		...
		for(size_t i=0; i<file_count; i++)
		{
			...
			if(test_file_type((char*)z_file_name, ".fb2") == TRUE)
			{
				...
				if(zf != NULL)
				{
					...
					fprintf(stderr, "Failed to unpack fb2 file: %s", z_file_name);
			}
		}
		zip_close(f);
	}
	else
		fprintf(stderr, "Failed to open zip file: %s; error code: %d", file_path, zip_error);
