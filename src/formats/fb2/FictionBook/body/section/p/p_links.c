...
int parse_fb2_p_link(FB2_READER_BOOK_VIEW* obj, xmlNode* parent_node, GtkTextIter* text_buff_end)		// своя функция для форматирования текста в угоду соблюдения стандарта
{
	...
	while(node != NULL)
	{
		...
		if(tag != NULL)
		{
			...
			parse_fb2_p_link(obj, node->children, text_buff_end);
