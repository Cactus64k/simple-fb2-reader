...
/*	int pos_y = 1000;
	GtkTextIter pos_iter;

	gtk_text_view_get_line_at_y(GLOBAL_FB2_READER.text_view, &pos_iter, pos_y, NULL);
	gtk_text_buffer_insert_with_tags(text_buff, &pos_iter,"11111111\n\n\n", -1, GLOBAL_FB2_READER.strong_tag, NULL);*/

	//GtkTextMark pos_mark;
	//gtk_text_buffer_get_iter_at_mark(text_buff, &pos_iter, &pos_mark);
	//gtk_text_view_scroll_mark_onscreen(GLOBAL_FB2_READER.text_view, &pos_mark);
