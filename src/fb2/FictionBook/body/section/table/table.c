...
int parse_table(FB2_READER_BOOK_VIEW* obj, xmlNode* parent_node, GtkTextIter* text_buff_end)
{
	...
	table_get_size(obj, parent_node, &cells_table, &cells_count);
	...
	table_put_line(obj, text_buff_end, cells_table, cells_count, "┌", "┬",  "┐");
			...
			if(strcmp((char*)node->name, "tr") == 0)
			{
				...
				while(td_node != NULL)
				{
					if(td_node->type == XML_ELEMENT_NODE)
					{
						if((strcmp((char*)td_node->name, "td") == 0) || (strcmp((char*)td_node->name, "th") == 0))
							p_get_text(td_node, cells_buff[cell_index++]);
					}
				...
				for(size_t i=0; i<=max_height; i++)
				{
					...
					for(size_t j=0; j<cells_count; j++)
					{
						...
						char* cell_string = cells_buff[j]->str;
