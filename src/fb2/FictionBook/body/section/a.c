...
int parse_formated_text_for_link(FB2_READER_BOOK_VIEW* obj, xmlNode* node, GtkTextIter* text_buff_end);
...
int parse_formated_text_for_link(FB2_READER_BOOK_VIEW* obj, xmlNode* parent_node, GtkTextIter* text_buff_end)		// своя функция для форматирования текста в угоду соблюдения стандарта
{
	g_return_val_if_fail(parent_node != NULL, -1);
	g_return_val_if_fail(text_buff_end != NULL, -2);
